...
sockets, request correlation, rate limits and reconnection. It knows
nothing about candles, features or trading decisions, and nothing here may
import from features/, regime/, reversal/, or strategy/.
...
THIS BUILD'S ONE DELIBERATE DEPARTURE FROM THE SIBLING EVEN/ODD CLIENT: the
Even/Odd engine keeps `quote` as text and never reparses it, because digit
extraction needs the exact trailing character and json.loads on "1234.50"
silently drops the trailing zero. Candle construction has the opposite
requirement -- OHLC arithmetic needs a real float -- so this client parses
`quote` once, at the one place it's read off the wire, and TickMessage
carries `price: float` rather than `quote_raw: str`. There is no repeat
parsing anywhere downstream; every candle and feature computation takes
that same float, so no rounding step is repeated at multiple layers.
"""
...
class DerivClient:
    ...
    @staticmethod
    def _parse_quote(tick: dict) -> float:
        q = tick.get("quote")
        try:
            return float(q)
    ...
    async def verify_rise_fall_available(self, symbol: str,
                                         currency: str = "USD") -> tuple[bool, str]:
        """Section 1 (reversal spec): refuse to trade a contract that is not
        actually offered. Verified against the live API rather than
        assumed, because an assumption here surfaces as a buy rejection
        mid-session.

        THE TWO FAILURE MODES ARE NOT THE SAME AND MUST NOT READ THE SAME.
        "Deriv confirmed CALL/PUT are not offered on this symbol" and "the
        contracts_for REQUEST failed, so nothing was confirmed either way"
        must not collapse into one message -- a lesson learned the hard way
        on the sibling Even/Odd bot's DIGITEVEN/DIGITODD check, where a
        rejected request field read in the logs like a negative
        determination about symbols that always offer the contract. The
        trading decision (skip this symbol) is the same either way; only
        the wording distinguishes the two cases here.
        """
        try:
    ...
    async def proposal(self, *, symbol: str, contract_type: str, amount: float,
        ...
        Properties not allowed: symbol. CALL/PUT (Rise/Fall) take no
        barrier -- unlike Higher/Lower, which this build does not support --
        and Deriv REJECTS a barrier field rather than ignoring it, so none
        is sent.
        """
        resp = await self._send({
